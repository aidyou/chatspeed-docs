...
一个专用的工具驱动的任务执行引擎，专为编程和复杂多步骤任务设计。Workflow 引擎不是简单的聊天会话扩展，而是一个基于 ReAct 循环的一等执行内核。

### 核心能力

- **工具驱动执行**：工作流通过工具调用、观察、审批点和显式完成提交来推进任务。
- **规划/执行模型分离**：为规划和执行使用不同的模型，平衡成本与质量。
- **子智能体编排**：派生子智能体并行处理子任务，支持调用模式和后台模式。
- **审批系统**：支持自动审批列表、智能 AI 审查、完全人工确认和最终审计模式。
- **上下文压缩**：智能分级压缩，按任务边界进行压缩，保留相关上下文的同时减少 Token 用量，实际编码测试中缓存命中率达约 96%。
- **记忆系统**：跨会话持久化的项目级和全局记忆，存储在项目目录外以避免 Git 追踪。
- **安全体系**：PathGuard 文件系统控制、工具白名单、Shell 策略和授权路径验证。
- **状态机**：完整的 13 个状态生命周期，包括等待用户输入、审批、子智能体等状态。

### 集成

- **Skills**：从用户安装 (`~/.chatspeed/skills`)、内置和工作流管理的来源加载技能包。
- **MCP**：直接集成 MCP 代理，支持网络搜索/抓取和已安装的 MCP 工具。
- **AGENTS.md**：通过 `AGENTS.md` 文件进行项目级智能体配置。
...
- **统一访问**：CCProxy 模块通过 `http://localhost:11435/mcp/http` 提供聚合后的所有 MCP 工具
...
3. **[Workflow 工作流引擎](../../workflow/)** - 了解工具驱动的任务执行引擎
