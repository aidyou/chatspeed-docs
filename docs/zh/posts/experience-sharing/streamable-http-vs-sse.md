...
目前 [Chatspeed](https://chatspeed.aidyou.ai) 的 `MCP` 代理已全面采用 `Streamable HTTP` 协议并实现了会话高效管理，能确保在网络断线或电脑休眠（7天内）会话自动恢复。自 v2.0 起，`Chatspeed` 作为 MCP 代理服务器时已不再提供 SSE 服务端入口，统一使用更稳定的 Streamable HTTP 协议。如果您的客户端仍使用旧的 SSE 入口，请将 `MCP` 代理地址的 `/mcp/sse` 改为 `/mcp/http` 即可
