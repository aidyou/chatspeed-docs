...
CCProxy 支持主流 AI 对话、Responses 与嵌入（Embedding）协议之间的无缝转换：

- **OpenAI 兼容格式**（对话与嵌入）
- **OpenAI 兼容 Responses API**（`/v1/responses`）
- **Claude API**
- **Gemini API**（对话与嵌入）
- **Ollama 本地模型**（对话与嵌入）
...
- 支持 OpenAI 兼容的 `POST /v1/responses` 接口。
- 当上游供应商声明 `supports_responses_api` 或 `supportsResponsesApi` 时，CCProxy 可以直通上游 `/responses` 端点。
- 如果上游并不原生支持 Responses，CCProxy 仍可将 Responses 请求转换到统一 chat 流水线，再把结果重新序列化为 Responses 兼容输出。

### 🔀 动态分组切换

- 为不同使用场景（如开发、生产、特定项目）定义独立的代理分组。
- 使用 `/switch` 前缀，始终路由到 Chatspeed 界面中当前设置为“激活”的分组。
- 无需重新配置客户端工具，即可在界面上一键切换所有接入工具的后端模型和注入规则。

### 🖥️ 服务器级切换与统计

- 除了分组级切换外，Chatspeed 还支持针对代理别名进行**服务器级切换**，可以直接调整每个代理服务所指向的后端模型目标。
- 代理切换窗口同时提供**按服务器聚合的 token 统计与趋势视图**，支持查看输入 token、输出 token、缓存 token、缓存命中率以及近期每日趋势。
