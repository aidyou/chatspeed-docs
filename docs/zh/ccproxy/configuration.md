...
分组切换有两种方式：
1. **快速切换面板**：按下 `Alt+Shift+P`（可自定义）呼出轻量级切换面板，即时切换分组
2. **设置页面**：进入代理设置，手动选择激活的分组

同一个轻量级窗口也支持**服务器级别切换**。在 `servers` 标签页中，您可以直接为每个代理别名调整其后端模型目标，而无需离开切换窗口。

此外，代理切换窗口还提供**按服务器聚合的 token 统计与趋势视图**，可查看输入 token、输出 token、缓存 token、缓存命中率以及近期每日趋势。
