...
Currently, [Chatspeed](https://chatspeed.aidyou.ai)'s `MCP` proxy has fully adopted the `Streamable HTTP` protocol with efficient session management that ensures automatic session recovery after network disconnection or computer hibernation (within 7 days). Since v2.0, Chatspeed no longer exposes an SSE server endpoint as an MCP proxy server. The `SSE` protocol has been fully removed from the server side and access is now unified on the `Streamable HTTP` transport at `/mcp/http`. If your client still uses the old SSE entry, simply change `/mcp/sse` to `/mcp/http` in the `MCP` proxy address.
